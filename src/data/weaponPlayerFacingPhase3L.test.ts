...
assert.ok(inspectWeaponPlayerFacing('aegis-longsword'));
...
const beforeDrawback = JSON.stringify(WEAPON_DRAWBACK_RECORDS['hex-carbine']);
const guidance = resolveAbilityGuidanceForWeapon('hex-carbine', 'SHRAPNEL_BLOOM' as never)
  ?? resolveAbilityGuidanceForWeapon(
    'hex-carbine',
    getWeaponLoadoutRecommendationProfile('hex-carbine').recommendations[1]!.abilityId,
  );
assert.ok(guidance === null || guidance.label.length > 0);
assert.equal(JSON.stringify(WEAPON_DRAWBACK_RECORDS['hex-carbine']), beforeDrawback);
...
assert.deepEqual(normalizeWeaponBriefAcknowledged(['bad', 'aegis-longsword']), ['aegis-longsword']);
assert.equal(hasWeaponBriefAcknowledged(undefined, 'aegis-longsword'), false);
const ack = withWeaponBriefAcknowledged([], 'aegis-paired-blades');
assert.ok(hasWeaponBriefAcknowledged(ack, 'aegis-paired-blades'));
const ack2 = withWeaponBriefAcknowledged(ack, 'aegis-paired-blades');
assert.equal(ack2.filter((id) => id === 'aegis-paired-blades').length, 1);
// Survives "new run" conceptually — acknowledgement is account-scoped, not cleared by re-normalize empty
assert.ok(hasWeaponBriefAcknowledged(normalizeWeaponBriefAcknowledged(ack2), 'aegis-paired-blades'));
...
assert.equal(isWeaponFamilyId('hex-carbine'), true);
assert.equal(isWeaponFamilyId('ash-shotgun'), false);

// --- First-use trigger semantics ---
{
  const id = 'aegis-paired-blades' as const;
...
// --- Read-only rendering proofs ---
{
  ...
  getWeaponEnemyMatchup('aegis-longsword', 'RIOT_VANGUARD');
...
// --- Older-save + run-lifecycle brief persistence ---
{
  ...
  assert.deepEqual(normalizeWeaponBriefAcknowledged(undefined), []);
  assert.equal(hasWeaponBriefAcknowledged(undefined, 'hex-shotgun'), false);

  const acknowledged = withWeaponBriefAcknowledged(
    withWeaponBriefAcknowledged([], 'aegis-longsword'),
    'hex-revolver',
  );
  ...
  const afterReopenAck = withWeaponBriefAcknowledged(acknowledged, 'aegis-longsword');
...
// --- Inspection does not alter equipped weapon / loadout recommendation data ---
{
  ...
  const beforeProfile = JSON.stringify(getWeaponLoadoutRecommendationProfile('aegis-longsword'));
  inspectWeaponPlayerFacing('aegis-longsword');
  resolveAbilityGuidanceForWeapon('aegis-longsword', 'DEMONS_LUNG');
  assert.equal(getEquippedWeaponForClass(progression, 'AEGIS'), beforeEquip);
  assert.equal(JSON.stringify(getWeaponLoadoutRecommendationProfile('aegis-longsword')), beforeProfile);
}
