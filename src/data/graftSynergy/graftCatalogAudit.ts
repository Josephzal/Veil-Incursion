...
export function buildGraftCatalogAudit(): GraftCatalogAuditEntry[] {
  const aegis = ALL_VEIL_GRAFT_IDS.map((id) => {
    ...
    return {
      ...
      compatibleRule: 'Non-ULTIMATE Aegis abilities (incl. STRIKE when capacity allows)',
      ownershipSource: 'SANCTUARY_RESIDUE_APPLICATION' as const,
      progressionRequirement: 'Class-rank capacity/socket unlock + Sanctuary residue cost',
      sponsorRequirement: null,
      ...
      descriptionRuntimeConsistent: id !== 'ECHO_GRAFT',
      mechanicallyComplete: id !== 'ECHO_GRAFT',
