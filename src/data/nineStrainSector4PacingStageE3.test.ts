...
import { SECTOR_4_CONVERGENCE_IDS, CONVERGENCE_IDS } from '../types/convergence';
import { CORE_IMPRINTS } from './nineStrain/strainRegistry';
import { AFTERIMAGE_CORE_IDS } from '../types/afterimage';

// Wave 4 pairs every prior-sector Strain with Gravemark and Shardskin, plus Gravemark x
// Shardskin itself (Impact Lattice) -- 15 pairs total, matching the 15 Sector 4 Convergences.
const S4_PARENT_PAIRS: ReadonlyArray<readonly [StrainId, StrainId]> = [
  ['COUNTERFATE', 'GRAVEMARK'],
  ['RITUAL_CADENCE', 'GRAVEMARK'],
  ['AFTERIMAGE', 'GRAVEMARK'],
  ['STILLPOINT', 'GRAVEMARK'],
  ['WOUNDWEAVE', 'GRAVEMARK'],
  ['FAULTLINE', 'GRAVEMARK'],
  ['SOULWAKE', 'GRAVEMARK'],
  ['COUNTERFATE', 'SHARDSKIN'],
  ['RITUAL_CADENCE', 'SHARDSKIN'],
  ['AFTERIMAGE', 'SHARDSKIN'],
  ['STILLPOINT', 'SHARDSKIN'],
  ['WOUNDWEAVE', 'SHARDSKIN'],
  ['FAULTLINE', 'SHARDSKIN'],
  ['SOULWAKE', 'SHARDSKIN'],
  ['GRAVEMARK', 'SHARDSKIN'],
];
...
console.log('Stage E.3 — Sector 4 pacing');
...
function runCohort(wave: 4) {
  ...
  function pickPreferred(state: NineStrainRuntimeState, cardIds: readonly string[]): string {
    ...
    if (hostileTrace) return hostileTrace.id;
  ...
  function simulate(seed: number, family: string, includeElite: boolean) {
    ...
    for (const node of path) {
      ...
      if (pending.kind === 'FIRST_OMEN_STRAIN') {
        ...
        const pick = options.find((id) => pairPick.includes(id)) ?? options[seed % Math.max(1, options.length)];
        if (!pick) {
          illegalOffer = true;
          continue;
        }
  ...
  for (const id of unlocked) assert.ok(reachableStrains.has(id), `wave4 strain ${id}`);
  for (const id of Object.values(CONVERGENCE_IDS)) assert.ok(reachableCv.has(id), `wave4 convergence ${id}`);
  for (const id of SECTOR_4_CONVERGENCE_IDS) assert.ok(reachableCv.has(id), `wave4 sector4 convergence ${id}`);

  console.log(JSON.stringify({ wave, report, reachableStrains: [...reachableStrains], reachableCv: [...reachableCv] }, null, 2));
  return report;
}

const wave4 = runCohort(4);
assert.ok(Object.keys(wave4).length === 9);
assert.equal(SECTOR_4_CONVERGENCE_IDS.length, 15);
assert.equal(Object.keys(CONVERGENCE_IDS).length, 36);

console.log('Stage E.3 — Sector 4 pacing passed');
