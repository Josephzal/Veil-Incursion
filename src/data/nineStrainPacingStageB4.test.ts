...
import { getSector1ProductionDefinitions, indexDefinitions } from './nineStrain/definitionCatalog';
...
console.log('Stage B.4 — pacing simulation');
...
function engineReady(state: NineStrainRuntimeState): boolean {
  ...
  for (const strain of SECTOR_1_STRAIN_IDS) {
...
function simulate(seed: number, family: string, includeElite: boolean) {
  let state = activateNineStrainAcquisition(createDefaultNineStrainRuntimeState(), {});
  ...
  for (const node of path) {
    ...
    const nextStrain = primary && primaryCores >= 2
      ? SECTOR_1_STRAIN_IDS.find((id) => id !== primary) ?? primary
      : primary;
    ...
    if (pending.kind === 'FIRST_OMEN_STRAIN') {
      const pick = (SECTOR_1_STRAIN_IDS[seed % 3]) as StrainId;
...
for (const family of CANONICAL_WEAPON_FAMILY_IDS) {
  ...
  for (let seed = 0; seed < SEEDS; seed += 1) {
    ...
    assert.equal(withElite.firstOmenByNode2, true, `${family} omen ${seed}`);
    assert.ok(withElite.maxStrains <= 3, `${family} cap ${seed}`);
    assert.equal(withElite.illegalOffer, false, `${family} illegal ${seed}`);
    assert.equal(withElite.firstOfferTwoCores, true, `${family} cores ${seed}`);
  }
  report[family] = stats;
  assert.ok(stats.engineD1 / SEEDS >= 0.9, `${family} engine D1 ${stats.engineD1}`);
  assert.ok(stats.developedD2 / SEEDS >= 0.9, `${family} developed D2 ${stats.developedD2}`);
  assert.ok(stats.noEliteEngine / SEEDS >= 0.9, `${family} no-elite engine ${stats.noEliteEngine}`);
  const mean = stats.acceptSum / SEEDS;
  console.log(`${family} accepts mean=${mean.toFixed(2)} min=${stats.acceptMin} max=${stats.acceptMax} engineD1=${(stats.engineD1 / SEEDS * 100).toFixed(1)}% convD2=${(stats.developedD2 / SEEDS * 100).toFixed(1)}%`);
}

console.log(JSON.stringify(report, null, 2));
console.log('Stage B.4 — pacing simulation passed');
