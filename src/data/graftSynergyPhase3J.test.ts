/**
 * Phase 3J — Sanctuary run-scoped grafts + Stage II-B depth capacity.
 */
...
// Unique catalog IDs — exactly 49
{
  ...
  listGraftCatalogAudit().forEach((e) => {
    assert.ok(
      e.ownershipSource === 'SANCTUARY_RESIDUE_APPLICATION'
      || e.progressionRequirement.includes('Sanctuary'),
    );
  });
}

// Class rank XP/history still extends through 20 (informational mastery)
{
...
// Stage II-B — capacity from run depth, not Class Rank
{
  ...
  assert.equal(getGraftSocketAccessForRunDepth(1).allowApexMasterwork, true);
  assert.ok(describeGraftCapacityProgression().some((l) => l.includes('MAX_RUN_GRAFT_CAPACITY=3')));
}

// Sanctuary grant path enabled — zero Residue still "affords"
{
  ...
  assert.equal(canAffordAnySanctuaryGraft('AEGIS', 9999), true);
}
...
// Valid Sanctuary apply — no Residue charge; failed apply keeps previous
{
  ...
  assert.equal(first.cost, graft.cost); // catalog cost retained, not charged
...
// Offer filter no longer Apex-locks by Class Rank
{
  const filtered = filterGraftOffersForRunDepth('AEGIS', ['NEUTRON_GRAFT', 'APEX_GRAFT'], 1);
...
// Compatibility: Blood-Mag on fixed basic (safety / mechanic — not rank)
{
...
// Sanitize over-cap + unknown (Depth 1 capacity = 1)
{
...
// Configs are Sanctuary-legal at depth capacity (not Safehouse loadouts)
{
  ...
  ALL_WEAPON_FAMILY_IDS.forEach((id) => {
    ...
    // requiredClassRank remains historical metadata on profiles.
    assert.ok(typeof p.configurations[0].requiredClassRank === 'number');
    [p.configurations[0], p.configurations[1]].forEach((cfg, cfgIndex) => {
      const runDepthBand = (cfgIndex === 0 ? 1 : 3) as 1 | 2 | 3;
      ...
      assert.ok(cfg.assignments.length <= getGraftSocketAccessForRunDepth(runDepthBand).capacity);
