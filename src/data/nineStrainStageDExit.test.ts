...
import { CONVERGENCE_IDS, SECTOR_2_CONVERGENCE_IDS, SECTOR_3_CONVERGENCE_IDS } from '../types/convergence';
...
// Stage D exit predates the Stage E.3 Sector 4 (Gravemark/Shardskin) Convergences — scope this
// gate to the 21 Convergences that existed at the Stage D boundary, not the live 36-Convergence
// catalog. `CONVERGENCE_IDS` itself now carries all 36 ids post Stage E.3.
const ALL_CV = [
...
function cvParents(id: string): string[] {
  ...
  return [FAULTLINE_CORE_IDS.STRESS_PATTERN, SOULWAKE_CORE_IDS.BORROWED_NERVE];
}
...
for (const family of CANONICAL_WEAPON_FAMILY_IDS) {
  for (const coreId of CORES) {
    ...
    rt.commitRootAction(weaponFamilyExecutionContext(family, {
      rootActionId: `d-exit-${family}-${coreId}`,
...
for (const family of CANONICAL_WEAPON_FAMILY_IDS) {
  for (const cv of SECTOR_3_CONVERGENCE_IDS) {
    const rt = strainRuntime();
    ...
    rt.runTurnStart();
    rt.commitRootAction(weaponFamilyExecutionContext(family, { rootActionId: `s3-${family}-${cv}` }));
  }
}

{
  const rt = strainRuntime();
  grant(rt, FAULTLINE_CORE_IDS.STRESS_PATTERN);
  grant(rt, SOULWAKE_CORE_IDS.HOLLOW_EDGE);
  grant(rt, CONVERGENCE_IDS.LIVING_FAULT);
  rt.syncHostileIntents([
  ...
  assert.equal(resumed.faultline.playerTurnIndex, snap.faultline.playerTurnIndex);
  assert.equal(resumed.soulwake.recordedWake, snap.soulwake.recordedWake);
  assert.equal(resumed.schemaVersion, 15);
}

console.log('Stage D exit passed');
