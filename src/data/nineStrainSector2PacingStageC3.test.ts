...
import { SECTOR_2_CONVERGENCE_IDS } from '../types/convergence';
...
console.log('Stage C.3 — Sector 2 pacing');
...
function runCohort(wave: 1 | 2) {
  ...
  function pickPreferred(state: NineStrainRuntimeState, cardIds: readonly string[]): string {
    ...
    if (wave === 2 && hostileTrace) return hostileTrace.id;
  ...
  function simulate(seed: number, family: string, includeElite: boolean) {
    ...
    for (const node of path) {
      ...
      const pair = S2_PARENT_PAIRS[seed % S2_PARENT_PAIRS.length];
      ...
      if (pending.kind === 'FIRST_OMEN_STRAIN') {
        ...
        const pair = S2_PARENT_PAIRS[seed % S2_PARENT_PAIRS.length];
        const pick = wave === 2
          ? (options.find((id) => pair.includes(id)) ?? options[seed % Math.max(1, options.length)])
...
console.log('Stage C.3 — Sector 2 pacing passed');
